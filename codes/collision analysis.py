...
for i in printables_list:
    ...
    if hash not in list_to_check:
        list_to_check.append(hash)
    else:
        collisions.append(hash)

if len(printables_list) == len(list_to_check):
    print("There are no hash collisions")
else:
    print("There are hash collisions")
    print("the collisions are :" , len(collisions), "out of " len(printables_list), "printable characters")
