# Hashing 8-bit ASCII inputs classically
# to find inputs that yield a hexadecimal
# output of 00. This will be used for
# complexity analysis when compared
...
# Collision inputs set
collision_inputs = set()

# Initialize set for finding ASCII input(s)
# for 00 hex digest
ascii_digest00 = set()

# Initialize set for finding ASCII input(s)
# for hex digest that starts with 0
ascii_digest0 = set()
...
for i in printables_list:
    ...
    """
    if collision, there may be more than 1 
    ASCII input that results in 00 hex digest
    """
    if hash_result == '00':
        ascii_digest00.add(i)

    """
    if collision, there may be more than 1 
    ASCII input that results in 0x hex digest
    """
...
if not ascii_digest00:
    print("There are no ASCII inputs that result in hex digest of 00")
else:
    print("The following ASCII input character(s) results in a hex digest of 00:", ascii_digest00)

if not ascii_digest0:
    print("There are no ASCII inputs that result in hex digest that starts with 0")
else:
    print("The following ASCII input character(s) results in a hex digest that starts with 0:", ascii_digest0)