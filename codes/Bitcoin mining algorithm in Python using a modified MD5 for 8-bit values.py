...
def mine(transaction,prefix_zeros):
    prefix_str='0'*prefix_zeros
    for nonce in range(MAX_NONCE):
        text = str(transaction)
        hash1 = state.process(ord(text))
        hash2 = state.finalize()
        hash = state.hex_digest()
        # print(hash)
        if hash.startswith(prefix_str):
            print("Ord value of input transaction is :" ,ord(text))
            print("However if")
            print("a bitcoin is mined with nonce value :",nonce)
            print("it has;")
            return hash
    print("Could not find a hash in the given range of upto", MAX_NONCE)

transactions='''A''' #can be modified but must remain only one ASCII character
difficulty = 1       #given that the hex digest is only 2 hex digits long, maximum difficulty is 2
import time as t
begin=t.time()
new_hash = mine(transactions,difficulty)
